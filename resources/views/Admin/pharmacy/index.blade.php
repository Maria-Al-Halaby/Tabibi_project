...
@section('title', __('Pharmacy Management'))
...
@section('content')
    <div class="page-header">
        <div>
            <span class="eyebrow">
                ...
                {{ __('Pharmacy Management') }}</span>
            <h1 class="page-title">{{ __('Manage your center’s pharmacy team from one clean workflow.') }}</h1>
            <p class="page-subtitle">
                {{ __('Add pharmacists, review their contact details, and keep pharmacy coverage organized without leaving the
                dashboard style used across the rest of the admin area.') }}</p>
        </div>
        ...
        <div class="helper-badges">
            ...
            <span class="helper-badge helper-badge--accent">
                ...
                {{ __(':count pharmacists', ['count' => number_format($pharmacists->count())]) }}
            </span>
        </div>
    </div>
    ...
    <div class="row g-4">
        <div class="col-12 col-xl-5">
            <section class="section-card form-panel h-100">
                <div class="toolbar-row">
                    <div>
                        <h2 class="section-heading">{{ __('Add a pharmacist') }}</h2>
                        <p class="section-copy">{{ __('Create a new pharmacy user directly for this center.') }}</p>
                    </div>
                </div>
                ...
                <form action="{{ route('Admin.Pharmacy.store') }}" method="POST" class="row g-3">
                    ...
                    <div class="col-12 d-flex justify-content-end">
                        <button type="submit" class="btn btn-tabibi">
                            <i class="fas fa-user-plus me-2"></i>{{ __('Add pharmacist') }}</button>
                    </div>
                </form>
            </section>
        </div>
        ...
        <div class="col-12 col-xl-7">
            @if ($pharmacists->isEmpty())<section class="section-card empty-state h-100">
                    <div class="empty-state__icon">
                        <i class="fas fa-user-doctor"></i>
                    </div>
                    <h2 class="empty-state__title">{{ __('No pharmacists have been added yet.') }}</h2>
                                ...
                                <div class="d-flex flex-column flex-lg-row justify-content-between gap-4">
                                    <div class="d-flex align-items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p class="record-card__copy mb-0">{{ __('Pharmacy team member') }}</p>
                                        </div>
                                    </div>
                                    ...
                                    <div class="toolbar-actions">
                                        ...
                                        <form action="{{ route('Admin.Pharmacy.destroy', $pharmacist->id) }}" method="POST"
                                            class="m-0"
                                            onsubmit="return confirm(@js(__('Delete this pharmacist?')))">
                                            @csrf
                                            @method('DELETE')
