...
@section('title', __('Edit Secretary'))
...
@section('content')
    <div class="page-header">
        <div>
            ...
            <h1 class="page-title">{{ __('Update secretary details without leaving the dashboard flow.') }}</h1>
    ...
    <section class="section-card form-panel">
        <div class="toolbar-row mb-4">
            <div>
                <h2 class="section-heading">{{ __('Edit secretary account') }}</h2>
                <p class="section-copy">{{ __('Refresh contact details or rotate credentials when the desk team changes.') }}</p>
            </div>

            <a href="{{ route('Admin.Secretary.index') }}" class="ghost-button">
                <i class="fas fa-arrow-left"></i>
                {{ __('Back to secretary team') }}</a>
        </div>
        ...
        <form action="{{ route('Admin.Secretary.update', $user->id) }}" method="POST" class="row g-3">
            ...
            <div class="col-12">
                ...
                <p class="field-note">{{ __('Only enter a new password if this secretary should use a new login.') }}</p>
            </div>

            <div class="col-12 d-flex justify-content-end">
                <button type="submit" class="btn btn-tabibi">
                    <i class="fas fa-floppy-disk me-2"></i>{{ __('Update secretary') }}</button>
            </div>
        </form>
    </section>
@endsection
