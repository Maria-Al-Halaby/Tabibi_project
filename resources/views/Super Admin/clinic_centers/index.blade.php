...
@section('title', __('Clinic Centers'))
...
@section('content')
    <div class="page-header">
        <div>
            <span class="eyebrow">
                ...
                {{ __('Clinic Centers') }}</span>
            <h1 class="page-title">{{ __('Manage every clinic center from one polished overview.') }}</h1>
            <p class="page-subtitle">
                {{ __('The listing now gives each center clearer structure, status visibility, and safer management actions.') }}</p>
        </div>

        <div class="helper-badges">
            <span class="helper-badge">
                <i class="bi bi-buildings-fill"></i>
                {{ __(':count centers', ['count' => number_format($clinicCenters->count())]) }}
            </span>
        </div>
    </div>
    ...
    <div class="toolbar-row">
        <div>
            <h2 class="section-heading">{{ __('Clinic network') }}</h2>
            <p class="section-copy">{{ __('Review center contact details, operational status, and edit paths in one place.') }}</p>
        </div>

        <div class="toolbar-actions">
            <a href="{{ route('SuperAdmin.clinicCenter.create') }}" class="btn btn-tabibi">
                <i class="bi bi-plus-circle"></i>
                {{ __('Add clinic center') }}</a>
        </div>
    </div>

    @if ($clinicCenters->isEmpty())<section class="section-card empty-state">
            <div class="empty-state__icon">
                <i class="bi bi-hospital"></i>
            </div>
            <h2 class="empty-state__title">{{ __('There are no clinic centers yet.') }}</h2>
            <p class="empty-state__copy">{{ __('Create the first center to start building the network.') }}</p>
        </section>
    @else
        <div class="row g-4">
                    ...
                    <section class="record-card">
                        <div class="record-card__header">
                            ...
                            <span class="status-pill {{ $clinic_center->is_active ? 'status-pill--success' : 'status-pill--danger' }}">
                                ...
                                {{ $clinic_center->is_active ? __('Active') : __('Inactive') }}
                            </span>
                        </div>
                        ...
                        <div class="toolbar-actions">
                            ...
                            <form action="{{ route('SuperAdmin.clinic_center.destroy', $clinic_center->id) }}"
                                method="POST"
                                onsubmit="return confirm(@js(__('Are you sure you want to delete :name?', ['name' => $clinic_center->name])))">
                                @csrf
                                @method('DELETE')
